...
def test_configured_command_routes_contribute_menu_commands():
    adapter = _adapter({
        "telegram_routes": {
            "command_routes": {
                "status": {"description": "Project status", "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}},
                "hidden": {"description": "Hidden", "show_in_menu": False, "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}},
            }
        }
    })

    assert adapter._configured_telegram_menu_commands() == [("status", "Project status")]
...
def test_configured_command_route_matches_names_and_aliases():
    adapter = _adapter({
        "telegram_routes": {
            "command_routes": {
                "status": {"aliases": ["ks"], "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}},
            }
        }
    })
...
async def test_configured_command_route_invokes_subprocess_and_sends_inline_keyboard(tmp_path: Path):
    ...
    adapter = _adapter({
        "telegram_routes": {
            "command_routes": {
                "status": {"handler": {"type": "subprocess", "argv": ["python3", str(script)], "timeout_seconds": 5}}
            }
        }
    })
    msg = SimpleNamespace(chat_id=-100, message_thread_id=42, message_id=7, text="/status")
    ...
    assert sent["text"] == "handled status"
...
async def test_configured_callback_route_denies_unauthorized_user_before_handler():
    adapter = _adapter({
        "telegram_routes": {
            "callback_routes": [
                {"prefix": "app:", "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}}
            ]
        }
    })
...
async def test_configured_callback_route_invokes_subprocess_and_answers(tmp_path: Path):
    ...
    adapter = _adapter({
        "telegram_routes": {
            "callback_routes": [
                {"prefix": "app:", "handler": {"type": "subprocess", "argv": ["python3", str(script)], "timeout_seconds": 5}}
            ]
        }
    })
    route = adapter._configured_callback_route_for("app:status")
    query = MagicMock()
    query.answer = AsyncMock()
    query.message = SimpleNamespace(chat_id=-100, message_thread_id=42, message_id=8, text="/app")

    await adapter._handle_configured_telegram_callback_route(query, "app:status", route, query_thread_id=42)

    query.answer.assert_awaited_once_with(text="done app:status")
    sent = adapter._send_message_with_thread_fallback.call_args.kwargs
