...
async def test_ensure_forum_commands_includes_configured_routes_before_core_commands():
    adapter = _make_test_adapter(extra={
        "telegram_routes": {
            "command_routes": {
                "status": {"description": "Project current status", "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}},
                "queue": {"description": "Project queue", "handler": {"type": "subprocess", "argv": ["python3", "adapter.py"]}},
            }
        }
    })
    ...
    with patch("hermes_cli.commands.telegram_menu_commands") as mock_menu:
        mock_menu.return_value = ([("new", "Start new session"), ("status", "Old configured copy")], 0)
        with patch("telegram.BotCommand") as MockBotCommand:
            def _make_cmd(name, desc):
                cmd = MagicMock()
                cmd.name = name
    ...
    assert [cmd.name for cmd in args[0][:3]] == ["status", "queue", "new"]
    assert args[0][0].description == "Project current status"
